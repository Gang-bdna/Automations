...
//	public static WebDriver driver = new ChromeDriver();
	...
	public static void main(String[] args) throws InterruptedException {
		...
		if(args.length==0){
			...
			BannerCheck.initConf(conf);
			BannerCheck.funCkc(driver);
			...
			System.out.println("	-checklist         Path to Elements list config file");
		}
		else {
			if(args.length>=1 && !args[0].equals("-help")){
				for(int i=0; i<args.length; i++){
					...
					else if(args[i].equals("-checklist"))
						conf.put("CheckList", args[++i]);
					else{
