...
public class IR_RepeatCreateCube {
	/**
	...
	public static void rcCubes(WebDriver driver) throws InterruptedException, IOException{
		...
		while(itr.hasNext()){
			RName = itr.next();
			//Create report
			IR_CreateReport.cReport(RName);
		    IR_UserConsole.wait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(By.id("report-iframe")));
		    IR_AttrCollect.attrClt("fieldlistContainer", driver);	  
//		    Utils.loading(IR_AttrCollect.attrClt("fieldlist", driver));
		    //drag and drop
//			acts = gcbs.get(RName);
